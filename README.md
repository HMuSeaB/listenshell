...
* 🛡️ **深层网络伪装**：
  ...
  * **封面图片与本地硬盘缓存**：使用 `cached_network_image` 统一图片加载，所有的封面图片请求自动附加定制的伪装 UA 和 Bearer Token。图片自动在本地硬盘进行持久化存储缓存，第二次打开秒加载，极大降低对有声书服务器的并发图片拉取频率，彻底打消因频繁加载封面而被服主后台风控的隐患。
